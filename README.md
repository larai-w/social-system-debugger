...
## アプリとして入れる（PWA） / Install as an app (PWA)

このサイトは PWA（インストールできるWebアプリ）です。入れておくと全画面で起動し、オフラインでも動きます。登録・料金は不要です。

- **iPhone / iPad（Safari）:** 共有ボタン（□に↑）→「ホーム画面に追加」
- **Android（Chrome）:** メニュー（⋮）→「アプリをインストール」
- **PC（Chrome / Edge）:** アドレスバー右端のインストールアイコン、またはメニュー→「インストール」
...
## Stage 5：仮説を検証する（設計のみ・未実装）

> ⚠️ **まだアプリに入っていません。** 現在のメニューにあるのは「教員向けガイド」「プライバシーポリシー」「よくある質問」の3つで、Stage 5 の入口はありません。以下は確定した設計で、公開時期は未定です。

アプリのメニューから、8場面の比較教材を別タブで開ける形を想定しています。初回予想、作用者の交換、条件別比較、予算付き介入、振り返り。Page 1〜4の入力値とは独立した説明用モデルで、現実の因果や人物の責任は判定しません。日英表示と学習記録のJSON保存・読み戻しを予定しています。

[モデルと学習の説明](docs/STAGE5.md)（設計文書）。オフライン利用も、実装したときに本体と同じ形で対応する想定です。
