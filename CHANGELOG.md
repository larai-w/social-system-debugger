...
- **週次シナリオ 2027-W02〜W05 補充（T93・Opus委任37件目）**: PAGE 5 素材第4弾「同じ火/毒/洪水も、あらかじめ開いていた条件のある街だけを壊す」（監査系）。W02=P1 断層×対立フレーム／W03=P2 予備削減／W04=P3 浅読み癖／W05=P4 開けた門で PAGE 1〜4 を1週ずつカバー。在庫は **2027-W05 まで（30週）**＝2027年2月頭まで無人ローテ可能。到達可能性 CI green・親レビュー（実名ゼロ・ja/en 完備）・確立済みの完全委任運用どおり（Issues #118）。
- **Dependabot 自動 rebase ワークフロー（直営）**: `.github/workflows/dependabot-auto-rebase.yml` を追加。Dependabot PR の opened/synchronize で `@dependabot rebase` を自動投稿し、手動の後始末（旧 TODO ☐12）を恒久自動化。なお既存 PR 8本はマイナー/パッチ=マージ済み・メジャー2本（typescript 7.x / cdk-nag 3.x）と #10 はクローズ済みで残件ゼロ。
...
- **METHODOLOGY 監査＝全式照合・ドリフトゼロ確認（T79）**: `docs/METHODOLOGY.md` の全数式・パラメータ定義・ゴール条件を `web/js/engine.js`・`web/js/ui.js` の実装と照合。**軽微な表記ずれ3点を修正、要判断事項ゼロ**（研究者向け「検証可能な設計」の約束を文書で裏付け）（T79=Opus委任31件目）。
- **DEVELOPMENT ja/en 現行化（T80）**: `docs/DEVELOPMENT.md`・`docs/DEVELOPMENT.en.md` を現構成に同期（`web/vendor/`・CSP・5本のスクリプト・Node 22・週次手順の自動化化）。**親レビューで版数誤記1件を修正**（Sonnet委任報告では見落とした箇所）（T80=Sonnet委任7件目）。
- **CHANGELOG T65〜T78 追記＋SECURITY.md 追加（T81）**: 「発信準備・完全委任運用スプリント（T65〜T78）」エントリ（+42行・既存無改変）と `SECURITY.md`（脆弱性報告窓口・対応方針・スコープ明記）を追加。公開リポジトリの OSS 衛生（T81=Sonnet委任8件目）。

### 自己改善の仕組み

- **ローテ失敗→作業指示書 Issue の自動起票（T82）**: `weekly-rotate.yml` に失敗フックを追加。在庫切れ・その他エラーで `gh issue create` が ITIL 問題票フォーマット（症状・切り分け・対処・貼るだけプロンプト）の Issue を自動起票。**永続化＋冪等**（同週に複数回失敗しても1件のみ）（T82=Opus委任32件目）。
- **鮮度見張りテスト＋`make vendor-check`（T83）**: `tests/freshness.test.mjs` を新規追加（fail4種＝sw版数・アプリ版数の DEVELOPMENT ja/en 整合、warn2種＝CHANGELOG/backfill の T番号乖離）と `make vendor-check`（Chart.js の upstream 最新版チェック）を追加。**テスト合計36件体制**。初回実行で Chart.js 4.5.1 の存在と CHANGELOG/backfill の実バックログ2件を即時検出（T83=Sonnet委任9件目）。
- **学びの台帳 `docs/learnings.md`＋ラチェット方針（T84）**: 本プロジェクトで遭遇したインシデント9件（IN-01〜09）を「発生→根本原因→恒久ガード」の形式で台帳化。**「バグ1件=恒久ガード1件」のラチェット方針**を CLAUDE.md に明記（同じ問題を繰り返さない仕組みの制度化）。台帳本体は Sonnet委任、方針行の追記は直営（T84=Sonnet委任10件目＋直営）。
