...
- ✅ タスク4（週替わりシナリオ）完了。
5. **タスク5 AWS配信基盤（次）**: `/infra` に CDK(TypeScript)。S3(非公開/OAC)+CloudFront。`web/` と `content/weekly/*.json` をホスティング。latest.json は短TTL(5分)、他は長TTL。`npm run deploy` で S3同期＋CloudFront無効化(latest.jsonのみ)。README に構成図(Mermaid)・選定理由・「Dockerを使わない理由」・コスト根拠・セキュリティ判断。**scenario.js の `CONTENT_BASE_URL='https://REPLACE_WITH_CLOUDFRONT_DOMAIN/content/weekly'` を実ドメインに差し替える箇所あり**。AWSアカウント/リージョン/プロファイルはプレースホルダ、cdk synth 通過まで（deployはユーザー）。
6. タスク6 計測拡充（app_platform 共通プロパティ等。weekly_*/share_*/card_saved/notification_optin は tasks 3-4 で導入済み）。
7. タスク7 CI/CD（GitHub Actions + AWS OIDC、最小権限、engine.js ユニットテスト、週次JSONスキーマ検証）。
