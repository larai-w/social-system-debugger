...
make gh-project        # Phase 1 + T1〜T92 を Issues/Milestones/Labels に一括バックフィル
```

- スクリプトは **冪等**（同名 issue はスキップ）。何度実行しても重複しない。
- 事前確認は `node scripts/gh-project-backfill.mjs --dry-run`。
- バックフィルされる構成:
  - **Milestones(3)**: Phase 1 ／ Strategy sprints (T1–T24) ／ Delegation sprints (T25–T92)
  - **Labels**: `area:app / infra / testing / content / docs / marketing` ＋ `process:ai-subagent`
  - **Issues(99)**: 全て英語・完了済みは closed(completed) で作成
