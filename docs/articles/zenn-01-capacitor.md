...
この反省から、**Console エラーゼロ確認を Playwright で自動化**しました。Chart.js が正常な場合と CDN がブロックされた場合の両方で、全タブ遷移・プリセット・スライダー操作を行い、`pageerror`/`console.error` がゼロであることを CI でも回しています。「インラインの `onclick` が全部実在の関数に解決すること」も静的テストにしました（地雷2の再発防止）。

## 結果

- Web版: URL・挙動とも従来どおり（Pages でそのまま配信継続）
- アプリ版: 同じコードベースから iOS/Android が出る。週替わりシナリオ・ローカル通知・触覚・共有シートだけがネイティブで追加
