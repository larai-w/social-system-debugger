...
| # | シーン | 作り方 |
|---|---|---|
| 1 | PAGE 2 ショック生存の緑バナー「あなたの街は、生き残った」 | 🛡冗長性確保都市 → ⚡ショック注入 |
| 2 | PAGE 1 ワイマール崩壊の赤バナー＋スローガンログ | ⚡ワイマール崩壊 プリセット |
| 3 | 週替わりシナリオカード（今週の挑戦） | ネイティブビルドで週次カード表示 |
| 4 | 発見ログ（数個アンロック済みの図鑑） | 数分操作後に 📖 発見ログ |
| 5 | 結果カード共有ポップ（3ボタン） | 判定確定 → ▶ この結果を送る |
| 6 | PAGE 3 認知ネットワークの毒入れ→回復 | 探索深度を上げる前後 |
