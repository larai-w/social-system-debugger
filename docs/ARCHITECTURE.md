...
実行時の**外部依存はゼロ**です: Chart.js v4 は `web/vendor/` にセルフホストされ（かつては唯一の CDN 依存でした）、
エージェントの可視化は素の Canvas に描画します。チャートライブラリの読み込みに失敗してもアプリはグレースフルに
劣化します（スライダー・タブ・共有は動き続ける）— これは祈るのではなく CI で表明されている性質です（§4 を参照）。
...
- **ブランチ保護。** `make protect` は web/infra の CI チェックが通った PR をマージ前に要求します（CodeQL は
  助言的で必須ではありません）。
- **自己検証付きのプロモアセット生成器。** `npm run gen:shot`（README ヒーロー画像）・`make store-shots`
  （App Store / Play Store 用スクショ6枚）・`make announce-cards`（X 告知画像4枚）はいずれも Playwright で
  *実エンジン*を駆動し、Console エラーが1件でもあれば中断します — つまり、テストを壊さずに値を偽装することは
  構造上できません。
