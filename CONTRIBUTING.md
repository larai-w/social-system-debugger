...
## コマンドの入口
`make help` で全タスクが一覧できる（`make check` / `make synth` / `make handoff` / `make aws-deploy` / `make aws-wire` …）。

## テスト・検証（CIと同じものをローカルで）
```bash
make check                        # ユニットテスト + 週替わりJSON検証
make synth                        # CDK 構成 + cdk-nag セキュリティ検査
make handoff                      # ★セッション終了前チェック（クリーン・未push・テスト）
```
...
- AWS 初期化は簡素化済み: `make aws-bootstrap`（初回）→ `make aws-deploy` → **`make aws-wire`**（CDK出力から GitHub Secrets/Variables と `web/config.js` を自動設定。要 `gh auth login`）。
- ユーザーが手を動かす設定（push / AWS / Capacitor 実機）の最新チェックリストは [`CLAUDE.md`](CLAUDE.md) 末尾。
