...
|---|---|---|---|---|
| ☐0 | **`git push`（未pushコミットあり）** | 1分 | 1分 | かんたん |
...
- 在庫は **W50（12/7の週）まで**。毎週月曜 0:00 JST に `weekly-rotate.yml` が自動切替。
- **在庫が無い週はワークフローが赤く失敗して通知が来る**＝それが「書き足して」のリマインダー。
  さらに `npm run check` が残り3週を切ると警告を出す（T42）。
  補充は Claude Code に「W51以降のシナリオを4週分作って」と頼めばよい（到達可能性の自動テスト込みで作る運用が確立済み＝T40）。
