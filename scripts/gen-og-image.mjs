...
c.fillStyle = BG;
c.fillRect(0, 0, W, H);
c.strokeStyle = 'rgba(0,212,255,.06)';
c.lineWidth = 1;
...
c.strokeStyle = 'rgba(0,212,255,.3)';
c.lineWidth = 1;
...
c.fillStyle = CYAN;
c.font = `bold 60px ${JP}`;
c.shadowColor = 'rgba(0,212,255,.6)';
c.shadowBlur = 18;
...
gauges.forEach(([label, v]) => {
  ...
  c.fillStyle = '#172340';
  c.fillRect(64, gy + 14, 620, 22);
  c.fillStyle = GREEN;
  c.fillRect(64, gy + 14, 620 * v, 22);
  gy += 74;
});
...
c.strokeStyle = GREEN;
c.lineWidth = 4;
c.strokeRect(-tw / 2 - 24, -34, tw + 48, 68);
c.lineWidth = 2;
c.strokeRect(-tw / 2 - 32, -42, tw + 64, 84);
c.fillStyle = GREEN;
c.fillText(txt, 0, 12);
c.restore();
c.textAlign = 'left';
