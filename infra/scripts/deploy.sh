...
# bash 3.2 (macOS 既定) では set -u 下で空配列の "${arr[@]}" 展開が unbound variable で落ちるため、
# 以降は PROFILE_ARGS の展開に ${arr[@]+"${arr[@]}"} ガードを付け、空配列でも安全に展開する。
...
echo "→ syncing web/ ..."
aws s3 sync "$ROOT/web/" "s3://$BUCKET/" ${PROFILE_ARGS[@]+"${PROFILE_ARGS[@]}"} --region "$AWS_REGION" \
...
aws s3 sync "$ROOT/content/" "s3://$BUCKET/content/" ${PROFILE_ARGS[@]+"${PROFILE_ARGS[@]}"} --region "$AWS_REGION" \
...
aws cloudfront create-invalidation --distribution-id "$DIST_ID" ${PROFILE_ARGS[@]+"${PROFILE_ARGS[@]}"} \
